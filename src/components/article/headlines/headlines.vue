<template>
  <div class="layer">
    ...
    <div class="ssdht">
      ...
      <div class="tianj1">
        <div class="tianj2">
          <el-button type="primary" @click="tianjiabj(1)" round>添加公告</el-button>
        </div>
      </div>
      <!-- 全局搜索组件 -->
    ...
    <div class="list-div">
      <div class="title">
        ...
        <div class="li">是否已读</div>
        <div class="li">操作</div>
      </div>
      <div class="title" v-for="(item,key) in gongData" :key="key">
        <div class="li">{{item.Id}}</div>
        <div class="li">{{item.NoticeTitle}}</div>
        <div class="li">
          <el-image style="width: 80px; height: 50px;padding-top: 6px;" :src="item.NoticeStreamer"></el-image>
        </div>
        <div class="li">{{item.NoticeCentent}}</div>
        <div class="li">{{item.CreateTime}}</div>
        <div class="li">{{item.IsRead}}</div>
        <div class="li">
    ...
    <div class="tankxg" v-show="juztk">
      <div class="biaotl">删除公告</div>
      <div class="wltw">确定删除“{{userTK.NoticeTitle}}”吗？</div>
      <div class="anntl">
...
export default {
  ...
  methods: {
    ...
    loadingData() {
      ...
      this.api.apiPost(
        "/OperationManager/SysNoticeList",
        "post",
        data,
        "",
        (status, code, res) => {
    ...
    tianjiabj(key, data) {
      if (key == 1) {
        ...
        this.$store.commit('gonStorage', data)
        this.$router.push({
          path: "/headlinestjxg",
          query: {
            key: key
